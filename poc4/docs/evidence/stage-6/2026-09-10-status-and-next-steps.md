...
本轮候选删除清单、文件 SHA-256、字节数和原报告统计记录于 [清理清单](2026-09-10-artifact-cleanup.json)：5 个过时报告目录与 2 份日志，共 16 文件 / 158,089 字节（154.38 KiB）。仅选择该 Stage 6 工作树内明确列出的旧日志/报告；已经核验绝对路径、reparse point、Git 跟踪状态与 checksum。**删除调用被执行策略拒绝，实际删除 0 文件、释放 0 字节，候选文件全部仍在；清理状态为 BLOCKED_BY_EXECUTION_POLICY，不能记为完成。** 未换机制绕过限制，不使用 git clean 或 mvn clean。
