...
**日期：** 2026-09-21
**状态：** 已确认产品方向与 MySQL 保留要求；本设计的具体默认值待整体审阅。尚未实施或进行运行验收。
...
| 事项 | 首版设计默认值 | 理由 |
| --- | --- | --- |
| 工作负载 | 普通 Java 使用 Job；Web 应用使用单副本 Deployment | 区分有限任务与持续服务 |
| 数据库部署 | 每项目独立 MySQL StatefulSet 与独立数据 PVC | 数据归属项目，不随应用 Run 删除 |
| Redis | 每项目独立缓存实例；首版不提供持久化模式 | 避免把缓存和 MySQL 数据保留承诺混为一谈 |
| 停止应用 | 保留依赖实例、凭据、数据和应用端点地址 | 修改后重启不重新初始化环境 |
| 公网入口 | 多端口 NodePort，由集群分配外部端口 | 延续当前入口方式；外部可达性另行实测 |
...
关闭工作台、退出登录或应用停止，都不等于删除项目。首版不增加“暂停整个环境”或数据库重置按钮。依赖在项目存续期间占用资源，容量预算必须包含它们。
...
创建表单增加模板、依赖与公开端口设置，提交的是产品配置，不接受任意镜像、Shell 命令、资源名、外部 NodePort 或 Kubernetes YAML。
...
```json
{
  ...
  "publicPorts": [
    { "name": "web", "targetPort": 8080 },
    { "name": "api2", "targetPort": 9090 }
  ]
}
```
...
- 0–3 个公开 TCP 端口；每项名称唯一，targetPort 唯一且处于 1024–65535。外部 NodePort 由集群分配，不允许前端指定。
- 首版 Web 模板主端口默认 8080；内部健康检查管理端口保留为 18081，不可选择为公开端口。配置额外端口仅建立转发，不会自动让 Java 代码多启动一个监听器。
...
- 采用 NodePort 不等于已公网可达。部署配置明确公开主机地址，并验证允许的 NodePort 范围、实际公网路由与防火墙。当前平台的单个 NodePort 可达不能证明新分配端口也可达。
- 分开记录“地址已分配”“应用就绪”“公网访问实测通过”；平台不把公网探测结果冒充每一时刻的可用性保证。
- 停止应用时保留 Service/NodePort 分配，不再有活动端点。重新启动采用新的 Run selector；意外丢失 Service 时不能假定分配地址不变，必须更新实际结果。
- MySQL、Redis 的 Service 均为内部服务；公开应用端口不改变这一点。workspace-agent、健康管理端口和平台后端接口不由此功能公开。
...
- 有限启动预算默认 1800 秒，涵盖依赖等待、拉镜像、构建和就绪。持续在线后不沿用任务的 30 分钟总执行上限；由用户停止、实际失败或明确的运维操作结束。
- `RUNNING` 表示进程运行；附加服务可用性 `STARTING` / `READY` / `UNAVAILABLE`，区分就绪和进程状态。没有活动 Run 时项目显示应用 `STOPPED`。
- Spring Boot 模板暴露内部 readiness 健康检查，选择了依赖时将相应健康检查纳入 readiness。管理端口的健康结果还必须确认主业务监听器已启动，不能仅凭独立管理端口存活判定应用可访问。依赖暂时不可用不直接成为 liveness 失败的依据，防止反复重启应用；不公开 env/config 等管理端点。
- Pod 不 Ready 时 Service 不发布可用端点；运行状态及每个公开端口的实际监听结果分别呈现。一次端口监听验证只证明连接能力，验收还必须请求预期业务响应。
- 编译失败或不能就绪在有限预算后失败；后续重新进入不可用状态的恢复观察也有 1800 秒预算。观察到应用反复重启时使用有限恢复预算（初版同一 Run 累计超过 3 次即终止），结合 Pod UID 变化和容器 restartCount 增量持久化计数，后端重启不能归零，避免换 Pod 绕过预算。检测有延迟，状态不能承诺重启次数的强同步上限。
- 服务正常退出也不是 `BUILD_SUCCEEDED`；未请求停止却退出时进入恢复观察并按预算恢复或记录服务失败。用户主动停止沿用 `CANCELLED / USER_STOPPED`，UI 对服务显示“已停止”。

停止流程：先持久化 STOPPING，撤销在线入口，停止/删除对应控制器并等待所有本次 Run 的 Pod 消失，收尾日志，再终结 Run 并解锁。优先优雅停止（默认 30 秒）；观察预算默认 120 秒。超过观察预算保留 STOPPING/RECOVERING 与写锁供继续核对，不能以请求超时推断工作负载已停止。

### 7.4 恢复、Pod 重建与日志

- 后端恢复先核对数据库意图、资源 UID、project/run 标签及控制器归属，再继续；存在 STOPPING/DELETING 意图时不重新启动服务。
...
MySQL 使用一实例一 PVC，数据挂载与工作区分离。正常替换 Pod、重启应用、后端恢复保留同一 PVC 和凭据，控制器缩容不触发删卷。存储类必须支持 MySQL 文件语义并在目标环境验证；旧 workspace 存储验证不能代替数据库写入及恢复验证。
...
建议持久化类别：项目的 template/execution kind；公开端口及实际分配；每项依赖的选择、状态与凭据引用；带组件、类型、名称、UID 和存储关联的资源清单；Run 的工作负载引用、服务可用性和日志来源。具体表拆分、索引和 API DTO 在实施计划中确定。

数据库迁移必须向后兼容已有项目/Run，不改写已有工作区；必须使用一次性 schema 验证，不能重置平台的两个运行 schema。前端在严格解析中增加新类型和原因码，不能通过放开任意字符串回避契约。

## 10. 创建失败、恢复与删除

创建流程先持久化规范化配置及项目身份，再按稳定名称创建 workspace、凭据、依赖、Service 和清单。写操作超时后根据身份和实际资源核对，禁止换 ID 盲重试；不能在响应丢失后重复申请一套 NodePort、数据库或新密码。
