...
**日期：** 2026-09-21
**状态：** 用户已确认异常退出后手动再次运行、不自动拉起。原 Job + Service 方案已有实施分支，实际验收以[运行时记录](../../../poc4/docs/evidence/java-runtime/acceptance.md)为准。2026-09-22 新增的 Maven 预置依赖与项目缓存方案已获用户确认，本次仅更新文档，尚未实现或测量提速。
...
- 2026-09-22 用户确认：构建 Java 运行镜像时预置标准 Spring Boot 模板依赖及构建插件；首次运行初始化项目自己的 Maven 缓存，后续缺少的依赖按需下载。缓存使用现有 workspace PVC 的独立目录，随项目保留和删除，不新增依赖缓存 PVC。具体契约见 §7.5。
...
### 7.5 Maven 预置依赖与项目缓存（2026-09-22 补充）

**目的：** 缩短点击 Run 到应用可访问的时间，并避免同一项目每次 Run 重复下载依赖。创建工作区不等待 Maven 下载、复制预置仓库或构建应用；进入编辑器的耗时与应用首跑分别测量。Maven 仍按项目 POM、传递依赖和构建插件解析，不下载所谓“全套 Spring Boot”。

**镜像预置：** Java 运行镜像包含固定模板所需的 Maven 仓库文件，覆盖 Web 无数据库、MySQL、Redis、两者同时选中四种组合及原 console 模板。预置包含 JAR、POM、父 POM/BOM 和实际运行所需插件；不会把镜像中的全部依赖加入用户项目 classpath。预置输入复用实际 `WorkspaceTemplate` 生成结果，版本变更需重建镜像并验证，不能维护另一套手抄 POM。Maven/JDK 沿用当前运行镜像版本，不借此升级工具链。

**存储与使用：**

| 位置 | 内容与访问方式 |
| --- | --- |
| 镜像 `/opt/manao-maven-seed/repository` | 平台构建的只读预置仓库；不挂载 PVC 覆盖此目录 |
| workspace PVC 根目录 `.manao-cache/maven/repository` | 项目自己的可写 Maven 缓存，不位于 `project-<projectId>` 代码目录中 |
| Run 容器 `/maven-cache` | 同一 workspace PVC 的 `.manao-cache/maven` subPath，Maven 显式使用 `/maven-cache/repository` |

workspace agent 继续只挂载代码 subPath，因此文件树、保存版本和源文件操作不包含缓存。MySQL 独立 PVC 保持原约定；缓存不新增 storage binding、数据库表、用户输入或共享依赖 PVC。缓存与代码共享现有卷容量，空间不足应报告失败，不自动删代码、数据库或整个缓存。

**执行与保留：** 首次 Run 在启动 Maven 前把预置文件复制到项目缓存，随后补齐用户 POM 所需的新依赖。以预置内容版本记录已完成初始化；同版本后续 Run 快速跳过复制。新预置版本只补缺、不覆盖项目已有文件；部分复制失败不写完成标记，文件以临时文件加原子发布避免遗留半个 JAR。缓存失败不得静默退回临时目录，进而声称已复用缓存。Maven 可按其规则检查元数据、更新 SNAPSHOT；“复用缓存”不承诺永不联网。

新项目准备目录，已有项目在第一次新 Run 前幂等补建目录与所需权限；不重建 PVC、不覆盖模板、不修改保存版本。应用运行身份沿用后端 `WORKSPACE_UID/GID`（当前为 10001），不能照搬镜像 Dockerfile 的 1001。准备操作限于缓存目录，不递归更改整个 PVC。初始化属于原启动预算；Web 在认领本 Run 之后才执行缓存准备及 Maven，监督进程继续是 PID 1，1800 秒启动预算和首次 Ready 后 7200 秒期限保持不变。console 同样使用持久缓存并保留原 1800 秒总时限。

停止、到期、失败、Pod 替换及后端重启均不主动清空缓存；项目删除沿用 workspace PVC 的完整回收流程。缓存可重建，不等同于 MySQL 数据保留承诺；首版不新增缓存重置 UI、清扫服务或跨项目写共享仓库。

**验证：** 在预拉指定镜像的节点上分别记录工作区可编辑、首次 Run、后续 Run 的时间；以真实缓存命中/无外部下载的证据证明机制，以实测数据评价提速。空项目缓存能从镜像初始化并完成标准模板运行；增加一个预置仓库没有的固定版本依赖后，首次下载、第二次复用；原有项目、非 root 权限、停止后保留和最终删除均需覆盖。正式运行允许下载新增依赖；离线模式只用于验证已预置内容完整。缓存复制不能绕过单次执行认领、停止、日志和期限检查。正式两小时验收并入原计划 Task 9，不增加第二轮两小时等待。

未来 Python/Go 可以沿用“项目 PVC 内专用缓存”的原则，各自交给包管理器处理；本次不实现其他语言或统一缓存框架。实施步骤、文件与测试位置见[Maven 缓存补充计划](../plans/2026-09-22-java-maven-cache-implementation-plan.md)。
