...
**日期：** 2026-09-21
**状态：** 已确认产品方向、MySQL 保留与独立 PVC、应用启动成功后的 2 小时上限，以及公网端口 30000–31000 范围；其余默认值与具体执行机制待设计审阅。尚未实施或进行运行验收。
...
- 用户要求公网开放端口范围限制为 30000–31000（含两端）；本次按公网侧 NodePort 理解，不将应用内部 targetPort 限制到该范围。
...
| 公网入口 | 多端口 NodePort；平台在 30000–31000 中自动选取并向 Kubernetes 显式申请 | 范围已确认；自动选择方式沿用建议默认值 |
...
- 0–3 个公开 TCP 端口；每项名称唯一，targetPort 唯一且处于 1024–65535。公网侧 NodePort 必须处于 30000–31000（含两端），两者是不同字段，例如公网 30081 → 应用 8080。
- 分配方式仍按建议默认值：用户选择要公开的应用端口，平台从限定池中选取外部端口并显式提交 nodePort；不再依赖省略 nodePort 后的集群全范围自动分配。前端手动挑选具体公网端口不是本轮已确认能力。
- 30000–31000 共 1001 个整数端口，不代表 1001 个全部可用。排除保留端口、已占用端口和分配中的端口；现有部署声明平台入口为 30080，必须保留，不能分给用户项目。Kubernetes 的集群级端口冲突检查是最后裁决，不能只检查当前用户或当前 namespace。
- 并发分配先持久化候选及申请状态，以稳定的 Service 身份申请；明确发生端口冲突才改选。创建超时/连接中断时先核对同一 Service，不能立即换端口重复创建；池耗尽返回明确错误，不越界到 31001 以后。保留多端口 Service 创建的整项结果，不向用户报告未实际分配的半套端口。
...
- 采用 NodePort 不等于已公网可达。部署前核对集群配置的 NodePort 范围包含 30000–31000，并验证这个范围内实际分配端口的公网路由与防火墙；此需求只限制 Manao 的分配池，不授权修改集群全局端口范围或其他服务。当前平台的单个 NodePort 可达不能证明新分配端口也可达。
...
建议持久化类别：项目的 template/execution kind；公开端口候选/分配状态与实际 NodePort；每项依赖的选择、状态与凭据引用；带组件、类型、名称、UID 和存储关联的资源清单；Run 的工作负载引用、firstReadyAt、不可变 expiresAt、7200 秒寿命策略快照、服务可用性和日志来源。具体表拆分、索引和 API DTO 在实施计划中确定。
...
- NodePort 池边界 30000/31000、越界 29999/31001、保留 30080、跨项目及跨 namespace 冲突、并发竞争、池耗尽、响应丢失后的核对；任何路径都不能分配越界或占用平台端口。
